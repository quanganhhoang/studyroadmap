    ...
    static async add(payload) {
        ...
        **/
        ...
        /*--- Collections synchronization ---
            - Will probably need to implement a db synchronization service (eventual consistency) when we are at scale
            - However, this should work decently for now!
        */
        ...
        await Promise.all(payload.milestones.map(milestoneId => {
            return Milestones.findOneAndUpdate({_id: milestoneId}, {
                "$addToSet": {
        ...
        // update each corresponding tags concurrently
        // for each milestone, attach the roadmapId to link the mileestone with this roadmap
        await Promise.all(payload.tags.map(tagId => {
            return Tags.findOneAndUpdate({_id: tagId}, {
                "$addToSet": {
                    roadmaps: roadmapDoc._id
                }
            }).exec();
